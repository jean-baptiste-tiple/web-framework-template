...
- Un commentaire `{/* … */}` dans le CORPS d'un .astro n'est pas opaque : une balise `<…>` citée dedans est compilée comme du vrai markup (un `<body>` cité dans BaseLayout a ouvert un second `body` et rendu le commentaire en texte sur toutes les pages, 2026-09-14). Les explications qui citent du markup vont dans le frontmatter (`//`). Contrôlable : `grep -rn -A5 "{/\*" src --include=*.astro | grep "<"` vide.
