...
- Contrôles post-déploiement (prod) : 1) 404 réelle : `curl -s -o /dev/null -w "%{http_code}" <domaine>/chemin-inexistant` → 404 (config host, voir seo-geo.md § Agentic). 2) Lighthouse sur l'URL de prod ≥ seuils de performance.md. 3) `npx is-agentic <domaine> --json` → zéro issue tier essential.
