...
- Pilote Open Kairos V1 LIVRÉE le 2026-09-14 (soir) : `C:\apps\site-open-kairos`, repo privé `jean-baptiste-tiple/site-open-kairos`, déployé sur `https://site-open-kairos.vercel.app` (noindex, URL provisoire). 6 pages + 2 légales + 404 composées depuis le kit (31 variantes + 3 layouts enregistrés, monde puchix), 8 offres / 5 cas / 2 experts en collections, formulaire RDV (service d'envoi à choisir : JB pense à Brevo). Mesures : Lighthouse 99-100 sur les 6 pages ; audit craft 16/20 (`fix` : deux défauts localisés corrigés en lot 5) ; is-agentic 69/100 (manques attendus : noindex, brand non indexée, Organization sans contactPoint/address faute de coordonnées, pas de négociation markdown). Reste côté client : chiffres de preuve, accords marques/témoignages, agrément CII, photos, coordonnées légales, logo (marques provisoires posées). Reste côté pipeline : P8 apprentissage (temps par phase, frottements) dans `docs/learnings.md`.
